...
import { Button, CalendarInput, Field, inputClasses } from '../../../components/ui';
...
const DateFilter = ({ id, label, value, min, max, onChange }: DateFilterProps) => (
  <Field label={label} htmlFor={id}>
    <CalendarInput id={id} type="date" className="min-w-0 max-w-full text-sm" value={value} min={min} max={max} onChange={(event) => onChange(event.target.value)} />
  </Field>
);
...
export default function AdminActivityLogsPage() {
  ...
  const copy = fr ? {
    ...
    refresh: 'Actualiser', filters: 'Affiner le journal', search: 'Rechercher', searchHint: 'Acteur, action, identifiant ou trace…', actor: 'Acteur', actorHint: 'Courriel de l’administrateur', action: 'Action', module: 'Module', from: 'Du', to: 'Au', anyDate: 'Toute date', allActions: 'Toutes les actions', allModules: 'Tous les modules', apply: 'Appliquer', reset: 'Réinitialiser', todayPreset: 'Aujourd’hui', weekPreset: '7 jours', monthPreset: '30 jours',
    ...
    refresh: 'Refresh', filters: 'Refine the log', search: 'Search', searchHint: 'Actor, action, identifier or trace…', actor: 'Actor', actorHint: 'Administrator email', action: 'Action', module: 'Module', from: 'From', to: 'To', anyDate: 'Any date', allActions: 'All actions', allModules: 'All modules', apply: 'Apply', reset: 'Reset', todayPreset: 'Today', weekPreset: '7 days', monthPreset: '30 days',
  ...
  const actorIdentity = (item: AuditLog) => {
    const user = item.userId ? data?.relatedUsers?.[item.userId.toLowerCase()] ?? data?.relatedUsers?.[item.userId] : undefined;
    return { name: user?.displayName || item.userEmail || copy.system, email: user?.email || item.userEmail };
  };
  ...
  const applyDatePreset = (days: number) => {
    ...
    const nextFilters = { ...filters, from: dateInputValue(from), to: dateInputValue(to) };
    setFilters(nextFilters);
    setQuery({ page: 1, pageSize, search: nextFilters.search.trim(), userEmail: nextFilters.userEmail.trim(), action: nextFilters.action, entityType: nextFilters.entityType, fromUtc: new Date(`${nextFilters.from}T00:00:00`).toISOString(), toUtc: new Date(`${nextFilters.to}T23:59:59.999`).toISOString() });
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <form onSubmit={applyFilters} className="rounded-[24px] border border-line/70 bg-surface p-5 shadow-[0_16px_40px_rgba(0,59,27,.045)] sm:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-12">
          ...
          <div className="grid min-w-0 gap-4 sm:grid-cols-2 md:col-span-2 xl:col-span-3">
            <DateFilter id="audit-from" label={copy.from} value={filters.from} max={filters.to || undefined} onChange={(value) => setFilters({ ...filters, from: value })} />
            <DateFilter id="audit-to" label={copy.to} value={filters.to} min={filters.from || undefined} onChange={(value) => setFilters({ ...filters, to: value })} />
          </div>
        </div>
        <div className="mt-5 flex flex-col gap-4 border-t border-line/55 pt-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-2" aria-label={fr ? 'Périodes rapides' : 'Quick date ranges'}>{[[1, copy.todayPreset], [7, copy.weekPreset], [30, copy.monthPreset]].map(([days, label]) => <button key={String(days)} type="button" onClick={() => applyDatePreset(Number(days))} className="min-h-10 rounded-full border border-line bg-surface-container/45 px-4 text-[9px] font-bold uppercase tracking-[.11em] text-green transition-colors hover:border-green hover:bg-green hover:text-white"><i className="ri-calendar-check-line mr-1.5" aria-hidden="true" />{label}</button>)}</div><div className="flex flex-wrap gap-3"><Button type="submit" disabled={loading}><i className="ri-filter-3-line" aria-hidden="true" />{copy.apply}</Button><Button type="button" variant="tertiary" onClick={resetFilters}>{copy.reset}</Button></div></div>
      </form>
      ...
      <section className="overflow-hidden rounded-[24px] border border-line/70 bg-surface shadow-[0_18px_45px_rgba(0,59,27,.05)]">
        ...
        {loading ? <div className="space-y-3 p-6" role="status"><span className="sr-only">{copy.loading}</span>{Array.from({ length: 6 }).map((_, index) => <div key={index} className="h-[76px] animate-pulse rounded-2xl bg-surface-container" />)}</div> : !data?.items.length ? (
          ...
          <div className="hidden overflow-x-auto lg:block">
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-line/70">{data.items.map((item) => <tr key={item.id} className="group transition-colors hover:bg-green/[.025]">
                ...
                <td className="px-5 py-4"><div className="flex min-w-[180px] items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green text-[10px] font-bold text-white">{initials(actorIdentity(item).name)}</span><span className="min-w-0"><strong className="block max-w-[230px] truncate text-sm text-ink">{actorIdentity(item).name}</strong>{actorIdentity(item).email && actorIdentity(item).email !== actorIdentity(item).name && <small className="block max-w-[230px] truncate text-[10px] text-ink-variant">{actorIdentity(item).email}</small>}</span></div></td>
          ...
          <div className="divide-y divide-line/70 lg:hidden">{data.items.map((item) => <article key={item.id} className="p-5">
            <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-green text-[10px] font-bold text-white">{initials(actorIdentity(item).name)}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-ink">{actorIdentity(item).name}</p>{actorIdentity(item).email && actorIdentity(item).email !== actorIdentity(item).name && <p className="truncate text-[10px] text-ink-variant">{actorIdentity(item).email}</p>}<time className="mt-1 block text-[11px] text-ink-variant">{formatDate(item.createdAtUtc)}</time></div><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${actionTone(item.action)}`}><i className={actionIcon(item.action)} aria-hidden="true" /></span></div>
      ...
      {selected && <div className="fixed inset-0 z-[80] flex justify-end" role="dialog" aria-modal="true" aria-labelledby="audit-detail-title"><button type="button" className="absolute inset-0 bg-ink/55 backdrop-blur-[2px]" aria-label={copy.close} onClick={() => setSelected(null)} /><aside className="relative h-full w-full max-w-xl overflow-y-auto border-l border-line bg-surface p-5 shadow-[-24px_0_70px_rgba(0,0,0,.18)] sm:p-8">
        ...
        <div className="mt-7 rounded-[22px] bg-green-deep p-6 text-white"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-gold font-bold text-green-deep">{initials(actorIdentity(selected).name)}</span><div className="min-w-0"><p className="truncate font-semibold">{actorIdentity(selected).name}</p>{actorIdentity(selected).email && actorIdentity(selected).email !== actorIdentity(selected).name && <p className="truncate text-xs text-white/65">{actorIdentity(selected).email}</p>}<p className="mt-1 text-xs text-white/65">{formatDate(selected.createdAtUtc)}</p></div></div><div className="mt-6 border-t border-white/15 pt-5"><span className="text-[9px] font-bold uppercase tracking-[.16em] text-gold">{copy.target}</span><p className="mt-2 font-display text-2xl font-bold">{humanizeEntity(selected.entityType)}</p></div></div>
